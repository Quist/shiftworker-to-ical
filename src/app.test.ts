...
const getDefaultInput = (): Shift[] => {
	return [{start: dayjs("2018-04-13 19:18"), end: dayjs("2018-04-13 22:18"), summary: 'test'}]
};
...
describe("Følger ICAL spesifikasjon", () => {
	describe("Overordnet", () => {
		...
		test("inkluderer PRODID", () => {
			expect(toIcal(getDefaultInput())).toContain("PRODID:-//hacksw/handcal//NONSGML v1.0//EN");
		});
	...
	describe("For VEvent", () => {
		...
		describe("dstart", () => {
			test("har en valid dato", () => {
				...
				expect(dtstamp).toEqual(dayjs("2018-04-13 17:18").format("YYYYMMDDTHHmmss[Z]"));
			});
		});
		...
		describe("dtend", () => {
			test("har en valid dato", () => {
				...
				expect(dtstamp).toEqual(dayjs("2018-04-13 20:18").format("YYYYMMDDTHHmmss[Z]"));
			});
		});
